...
"""
function estimators(data::DiscreteTimeData, Δ::Int=floor(Int,log(length(data))))::Tuple{Float64,Float64,Float64}
    if Δ == 0
...
function estimators(data::DiscreteTimeData, Δvec::Vector{Int})::Tuple{Float64, Float64, Vector{Float64}}
    N, T = size(data)
    X = data.X
...
"""
function fit(::Type{MarkovChainModel}, data::DiscreteTimeData, r₊::Float64; Δ::Int=floor(Int,log(length(data))))::MarkovChainModel
...
function Φ_aux(m,v,w_or_d,r₊)::Tuple{Float64,Float64,Float64}
    ...
    Φ₃ = 1/ϕ₂
    distance2admissibleset(Φ₁, Φ₂, Φ₃) == 0 || @debug "Estimators were clipped to admissible values."
...
function Φ(m,v,w,r₊)::Tuple{Float64,Float64,Float64}
    ...
    if r₊ > .5
        d = d₋
        0 < d < 1 || @debug "Careful. r₊="*string(r₊)*", d₋="*string(d₋)*" and d₊="*string(d₊)
        return Φ_aux(m,v,d,r₊)
    end
    if 4*r₊*r₋ - κ < 0
        d = d₋
        d > 1 || @debug "Careful. 4*r₊*r₋ - κ="*string(4*r₊*r₋ - κ)*", d₋="*string(d₋)*" and d₊="*string(d₊)
        return Φ_aux(m,v,d,r₊)
    end
    if d₊ > 2*r₋
        d = d₋
