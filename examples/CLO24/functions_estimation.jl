...
function estimatorsarray(
    ...
    id::Int=1,
    idmax::Int=1,
)::Tuple{Array{Float64,4},Array{Float64,2}}
    ...
    @progress "estimatorstable, i=" * string(id) * " on " * string(idmax) for idsimu in
                                                                              1:Nsimu
...
function estimatorstable(
    ...
    default_values::@NamedTuple{
        N::Int64, r₊::Float64, β::Float64, λ::Float64, p::Float64, Δ::Int64, Nsimu::Int64
    },
    tvec,
    ...
    if Paramsymbol == :Δ
        ...
        for idparam in eachindex(Paramvec)
            ...
            model = MarkovChainModel(β * λ, λ, p)
            E_new, E_inf_new = estimatorsarray(
                model, N, r₊, Nsimu, tvec, [Δ]; id=idparam, idmax=length(Paramvec)
            )
            E[:, :, :, idparam] = E_new[:, 1, :, :]
...
function complete!(df, E, Paramsymbol, Paramvec, default_values, tvec)
    ...
    return metadatacomplete!(df, Paramsymbol, default_values)
end
...
function complete!(df_inf, E_inf, Paramsymbol, Paramvec, default_values)
    ...
    return metadatacomplete!(df_inf, Paramsymbol, default_values)
end
...
function estimators2errors(df::DataFrame)::DataFrame
    ...
    metadata!(
        output,
        "Caption",
        "Values of the absolute difference between the estimator and the theoric value",
    )
    return output
end
...
function columnquantile(df::DataFrame, q::Real)
    ...
    metadata!(
        ...
        "Caption",
        string(100 * q) *
        "% quantile of the absolute difference between the estimator and the theoric value",
    )
    return output
end
...
function ploterrors(df::DataFrame, df_inf::DataFrame; targetstring::String="all")
    if targetstring == "all"
        ...
        plot(
            ...
            df[:, targetstring];
            group=df.parameter,
            color=indexin(df.parameter, paramvec),
        )
        scatter!(
            maximum(df.T) * ones(size(df_inf)[1]),
            df_inf[:, targetstring];
            group=df_inf.parameter,
            color=indexin(df_inf.parameter, paramvec),
            marker=:hline,
            label=false,
            markerstrokewidth=5,
        )
        xlabel!("T")
        ylims!(0, min(maximum(df[:, targetstring]), 0.3))
...
function ploterrors(
    ...
    dfs_inf::Tuple{DataFrame,DataFrame,DataFrame};
    targetstring::String="all",
)
    if targetstring == "all"
        for ts in ["m̂", "v̂", "ŵ", "μ̂", "λ̂", "p̂"]
        ...
        for param in paramvec
            push!(
                dflowup_inf,
                (Tmax, param, Array(dflow_inf[dflow_inf.parameter .== param, Not(1)])...),
            )
            push!(
                dflowup_inf,
                (Tmax, param, Array(dfup_inf[dfup_inf.parameter .== param, Not(1)])...),
            )
        end
        plot(
            df.T,
            ...
            color=indexin(df.parameter, paramvec),
            ribbon=(
                df[:, targetstring] .- dflow[:, targetstring],
                dfup[:, targetstring] .- df[:, targetstring],
            ),
        )
        scatter!(
            maximum(df.T) * ones(size(df_inf)[1]),
        ...
        plot!(
            dflowup_inf.T,
            dflowup_inf[:, targetstring];
            group=dflowup_inf.parameter,
            color=indexin(dflowup_inf.parameter, paramvec),
            label=false,
            alpha=0.5,
            linewidth=7,
        )
...
function plotestimatorserrors(
    df::DataFrame, df_inf::DataFrame; quantiles::Tuple{Real,Real}=(0.5, 0.5)
)
    plow, pup = quantiles
    ...
    return ploterrors((qlow, q50, qup), (qlow_inf, q50_inf, qup_inf))
end
...
function simulationandsave(
    ...
    CSV.write(
        "data/CLO24/estimators_vary_" *
        paramstring *
        "_delta_" *
        string(default_values.Δ) *
        ".csv",
        df,
    )
    open(
